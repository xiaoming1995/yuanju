...
- 在静态合盘基础上补充“缘分时长评估”，输出 `3个月`、`1年`、`2年以上` 三个窗口的关系维持概率等级与阶段说明
- 支持登录用户查看和复用合盘历史，且不污染现有单人命盘历史
- 尽量复用既有的 `pkg/bazi` 起盘能力、AI provider、Prompt 管理与前端展示模式

**Non-Goals:**
- 不在第一版实现择日、结婚年份预测、精确到具体月份/日期的分手时间预测
...
输入双方出生信息
  ...
  -> duration window engine 输出 3个月 / 1年 / 2年以上 维持概率等级 + reasons
...
### D3.5：新增独立的“缘分时长评估”输出，而不是精确断语

**选择**：在四维静态合盘之外，额外输出：

- `duration_assessment.overall_band`: `short_term` / `medium_term` / `long_term`
- `duration_assessment.windows`
  - `three_months`
  - `one_year`
  - `two_years_plus`
- 每个窗口使用 `high` / `medium` / `low` 等级，而不是直接向用户暴露伪精确分值
- `duration_assessment.summary`
- `duration_assessment.reasons[]`

底层可以保留内部数值或权重，但对外 API 与 UI 首版只展示等级与解释。

**Why**：
- 用户问的是“能维持多久”，需要动态阶段感，而不是把静态合盘硬解释成时间判断
- 直接说“三个月”“一年”会落入伪精确断语，产品风险高
- 用时间窗口 + 等级表达，既能保留可解释性，也为后续更细阶段分析留出空间

**放弃**：
- 直接预测具体分手时间
- 用单个“长期概率”替代阶段化窗口，解释力不够
...
### D7：时长评估采用“静态合盘 + 时间窗口同步分析”双层结构

**选择**：时长评估不只看静态原局，也不直接复用现有单人大运解读文案链路，而是增加一个独立窗口分析层。

建议链路：

```text
静态合盘结果
  + 双方未来阶段窗口（3个月 / 1年 / 2年以上）
  -> duration_assessment
```

第一版重点关注：
- 原局是否属于“强吸引但不稳”
- 双方在短中期窗口内是否同步进入有利关系承接的阶段
- 关键婚恋位的冲克风险是短期爆发还是持续存在
- 一方被显著触发、另一方却进入疏离窗口的失衡风险

**Why**：
- 用户要的是“关系维持性”，不是单纯静态匹配
- 保留独立窗口层，后续才能继续扩展到更细的阶段走势，而不重写合盘顶层结构

**放弃**：
- 直接复用单人 `liunian` / `dayun` 解读输出作为合盘时长结论
- 一开始就做月级别长期轨迹图
...
- **[时长评估伪精确]** 若直接输出“具体几个月分开”，容易让产品显得武断 → 第一版只允许窗口等级与解释，不允许具体日期断语
