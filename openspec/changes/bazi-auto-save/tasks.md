...
- [x] 3.2 手动使用游客不带任何身份操作进行起盘。确认返回正常计算数据，验证控制台及数据库未新增表列项产生；确保不受 401 身份拒绝异常骚扰。
- [x] 3.3 使用登陆态界面起盘。操作响应后点击导航查验"本账户排盘历史"并可见此套入座记录顺利诞生存在。
- [x] 3.4 登录后连续二次、三次对相同输入参数再次发力计算请求操作，以验证 DB 冲突机制的稳定及避免生成无效无尽重复冗杂账单数据积累；确认列表仍仅有一列最新纪录代表呈现即表明验证彻底结束。
