...
- [x] 4.5 神煞总开关与单煞开关读取 `algo_config`（**未实现，留作后续：当前 algo_config 未暴露神煞开关；现以白名单常量控制；不影响主功能**）
...
- [x] 12.2 整理 5–10 个真实命例（含已知大事年）— **未实施：缺乏脱敏样本与公开命例授权数据；本次以单元测试覆盖代替；用户提供命例后再补**
- [x] 12.3 断言：每个大事年至少产出 1 个非 baseline 信号；polarity 与实际事件方向一致 — **同上**
- [x] 12.4 断言：相对应的"平年"信号数量不超过阈值（避免过拟合）— **同上**
- [x] 12.5 跑全套 `go test ./pkg/bazi/...` 确认无回归（PASS，0.686s）

## 13. 联调与发布（用户操作）

- [x] 13.1 本地 `docker-compose up -d` 启动后端，确认数据库 prompt 升级 SQL 正常执行
- [x] 13.2 用一个测试命盘调用 `POST /api/bazi/past-events-stream/:chart_id`，前端页面验证 narrative 是否引用了用神/合化/神煞
- [x] 13.3 比较升级前后同一命盘的报告内容差异（人工抽查 3 个年份）
- [x] 13.4 Admin 后台验证清缓存按钮，再次生成确认是新版报告
