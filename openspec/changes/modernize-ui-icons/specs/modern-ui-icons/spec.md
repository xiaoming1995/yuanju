...
系统前端 SHALL 全面禁止使用 Emoji（如 命理、历史、用户、）作图标素材，改为使用 `lucide-react` 图标库中的专业矢量图标。
...
- **THEN** 标题前缀从 、清单 统一变更为微小的、发光的菱形 `Diamond` 或等效的仪表盘科技感极简单元素。

### Requirement: 五行徽章几何化体系

涉及五行属性渲染的组件（如 `YongshenBadge`）SHALL 停止使用具象 Emoji（木水火土），统一更换为着色的 Lucide `Hexagon` 图标，保持对原有五行基色的引用。

#### Scenario: 渲染喜用神徽章
- **WHEN** `YongshenBadge` 渲染用户的五行喜用神（例如"水"）
- **THEN** 界面上展示蓝色的 `Hexagon` 几何图标，不再出现具象的水滴 Emoji 水。
