...
func TestCalcFuyiStrength_NoRoot_Strong_BijieYin(t *testing.T) {
	...
	// 丙火的印=甲(偏印)或乙(正印)
	// 年支藏干主中=[甲,丙]→甲=偏印通过，时支同=甲→偏印通过 → 地支2个印→印旺
	...
	isStrong2, reason2 := calcFuyiStrength(
		...
		"丙", "丙", "丙", // 3个比肩
		[]string{"庚"}, // 申（年）：无火根
		[]string{"甲"}, // 寅（月）：甲=偏印，但是寅主气甲是木，ganWuxing[甲]=mu≠huo，所以不是丙的根 通过
		[]string{"庚"}, // 申（日）：无火根
		[]string{"甲"}, // 寅（时）：甲=偏印
	)
	// 有根检测：年申[庚]→庚=金≠火；月寅[甲]→甲=木≠火；日申[庚]→金；时寅[甲]→木 → 无根 通过
	// 比劫：天干3个丙=比肩 + 藏干中无火 = 3 ≥ 3 通过
	// 印：地支主气[庚,甲,庚,甲]→甲=偏印×2（月时）→ 地支2个印→印旺 通过
	if !isStrong2 {
