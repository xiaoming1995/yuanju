...
func GetPillarsShenSha(yg, yz, mg, mz, dg, dz, hg, hz string) [4][]string {
	...
	// 验证：1996命盘，日干乙→子午，年支=子，命中年柱太极贵人 通过
	// ══════════════════════════════════════════════════════════
	for i, z := range zhis {
		addIf(i,
			// 年干基准
	...
	for i, z := range zhis {
		...
		// 推导来源：1996命盘 庚(金)寅(木)→金克木 → 月柱墓门 通过
		//         1995命盘 丙(火)戌(土)→火生土 → 月柱无墓门 通过
		//         1996命盘 丙(火)子(水)→水克火(地克天,非天克地) → 年柱无墓门 通过
		ganWx := ""
		switch {
	...
	// ══════════════════════════════════════════════════════════
	// 第四+组：年干 → 四柱地支（流霞）
	// 流霞煞，与神煞汇总表对齐（甲酉乙戌丙未丁申戊巳己午庚辰辛卯壬亥癸寅）
	// ══════════════════════════════════════════════════════════
	...
	// ══════════════════════════════════════════════════════════
	// 第七+1组：童子煞
	// 规则A: 日支∈{子午卯酉} → 日柱(i=2) 得 童子煞
	// 规则B: 年支∈{子午卯酉} → 时柱(i=3) 得 童子煞
	// 验证1995: 日支=子→日柱通过; 年支=亥→时柱无通过
	// 验证1996: 日支=亥→日柱无通过; 年支=子→时柱通过
	// ══════════════════════════════════════════════════════════
	if strings.Contains("子午卯酉", dz) {
		addIf(2, true, "童子煞") // 规则A: 日柱
	}
	if strings.Contains("子午卯酉", yz) {
		addIf(3, true, "童子煞") // 规则B: 时柱（由年支触发）
	}

	// ══════════════════════════════════════════════════════════
	// 第七+2组：十灵日（固定干支表，查日柱）
	// 十灵日 = 甲子/甲午/甲申/甲戌/甲辰/甲寅 (6甲) + 乙亥/乙卯/乙丑/乙未 (4乙) = 10
	// 验证1996: 日柱=乙亥 ∈ 十灵日 通过
	// 验证1995: 日柱=丙子 ∉ 十灵日 通过
	// ══════════════════════════════════════════════════════════
	tenLingDays := map[string]bool{
		"甲子": true, "甲午": true, "甲申": true,
		"甲戌": true, "甲辰": true, "甲寅": true,
		"乙亥": true, "乙卯": true, "乙丑": true, "乙未": true,
	}
	dayGanZhi := dg + dz
	if tenLingDays[dayGanZhi] {
		addIf(2, true, "十灵日") // 十灵日仅标记日柱
	}

	// ══════════════════════════════════════════════════════════
	// 第七+3组：词馆（日干基准 → 四柱地支）
	// 甲→寅戌, 乙→卯亥, 丙丁→巳丑, 戊己→午辰, 庚辛→申子, 壬癸→酉未
	// 验证1996: 日干乙→{卯亥}, 日支=亥→日柱词馆 通过
	// 验证1995: 日干丙→{巳丑}, 四柱无巳丑→无词馆 通过
	// ══════════════════════════════════════════════════════════
