...
- ✓ "此步大运忌神透出，事业财运均有下滑风险，宜守不宜攻。"`,
	...
	// 增量迁移 (kb-gejv-system-mingge)：仅升级旧默认值，不覆盖管理员自定义内容
	if _, err := DB.Exec(
		`UPDATE ai_prompts
		 SET content = $1, description = $2, updated_at = NOW()
		 WHERE module = 'kb_gejv' AND TRIM(content) = $3`,
		systemMingGeKBGejv,
		"格局解释规则（以系统定格结果为准）",
		strings.TrimSpace(legacyKBGejv),
	); err != nil {
		log.Printf("增量迁移 (kb-gejv-system-mingge) 失败: %v", err)
	}
