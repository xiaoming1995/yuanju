...
test('personality helper exposes match type used by history + result pages', () => {
  const { getPersonalityMatchType } = loadPersonalityHelpers()

  assert.equal(typeof getPersonalityMatchType, 'function')
  assert.match(
    getPersonalityMatchType({ attraction: 85, stability: 50, communication: 50, practicality: 60 }),
    /高吸引高消耗型/,
  )
})
...
test('compatibility result new layout renders deep analysis after verdict section', () => {
  ...
  // section order: Verdict (with scores) → DeepAnalysis
  ...
  // personality is now generated by the AI report (LLM), rendered inside DeepReportNarrative
