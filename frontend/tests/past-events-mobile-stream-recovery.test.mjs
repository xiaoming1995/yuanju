...
test('PastEventsPage fences stale stream callbacks by generation request id', () => {
  ...
  assert.match(page, /const initialStreamStillCurrent\s*=\s*Object\.entries\(initialGenerationRequestIds\)/)
  assert.match(page, /current\?\.generation\?\.source\s*===\s*'initial'\s*&&\s*current\.generation\.requestId\s*===\s*requestId/)
  assert.match(page, /if \(!initialStreamStillCurrent\) return/)
