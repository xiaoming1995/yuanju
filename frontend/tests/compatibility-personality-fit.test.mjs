...
test('compatibility result shows personality fit before scores and professional details', () => {
  ...
  const professional = page.indexOf('<details className="compatibility-professional-details"')

  assert.ok(personality > dashboard, 'personality section follows decision dashboard')
  assert.ok(validation > personality, 'validation plan follows personality section')
  assert.ok(score > validation, 'score overview follows personality validation')
  assert.ok(professional > score, 'professional details remain lower priority')
  assert.match(page, /性格相处画像/)
  assert.match(page, /7 天观察/)
  assert.match(page, /30 天验证/)
