...
**全局放弃的"新发明"**：
- [NO] SnapStrip 横滑卡带（用户明确反对，希望"一眼看完"）
- [NO] tap-to-zoom 单柱放大模态（同上）
- [NO] 卡片化四柱（破坏传统横看一行/纵看一柱的阅读习惯）
- [NO] "全表/卡片"视图切换 toggle（无视图切换需求）

**全局保留的"新东西"**：
- [OK] 章节胶囊条吸顶 + 章节横向 scroll-snap 切章（章 ↔ 章之间的翻页交互）
- [OK] URL hash 双向同步（`#ming` / `#xing` / `#yun` / `#nian` / `#shu`）
- [OK] 章末快闪提示 `──→ 滑向《下章名》` + 浮动回顶按钮
- [OK] 桌面 ≥1024px 线性滚动 fallback + 章节锚点条
- [OK] 标题层级 H1·24 / H2·18 / H3·15 全局收口（替换现 18/20/22 混用）
