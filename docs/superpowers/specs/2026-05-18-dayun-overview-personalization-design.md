...
interface DayunTimelineProps {
  ...
  tiaohou?: { summary?: string; missing_gans?: string[] } | null
}
```
...
```
tiaohou.missing_gans 缺数据
  ↓
  skip（第三句省略）

tiaohou.missing_gans 有数据
  ↓
  missing 五行集合 M（从 missing_gans 推导）
  ↓
...
- `missingWuxing` 从 `tiaohou.missing_gans[0]` 经 `GAN_WUXING` 推导（取第一个）
...
| tiaohou 空 | 仍计算 | 仍计算 | 仍计算 | skip | 只 2 句 |
