...
- [x] **P0-1 历史页加载失败被静默吞掉，伪装成"空历史"**（S）[OK] 已修（2f95ab7）
...
- [x] **P1-1 路由零代码分割，23 个页面打进单个 1.63MB chunk（gzip 497KB）**（M）[OK] 已修（902ddfe）
...
- [x] **P1-2 html2canvas / html-to-image 打进首屏 bundle**（S，建议与 P1-1 一起做）[OK] 已修（902ddfe）
  导出库只在点击"导出"时才需要。
...
- [x] **P1-3 HistoryPage 分页缺失**（M）[OK] 已修（6c5fd62）
...
- [x] **P1-4 admin_handler.go 在 handler 层直接写 SQL（违反 AGENTS.md:140 硬规）**（M）[OK] 已修（27597c3）
...
- [x] **P1-6 前端 src 零单测**（M 起步）[OK] 已修（8a3961f）
...
- [x] **P2-1 401 硬跳转整页 reload**（S）[OK] 已修（d829663）
...
- [x] **P2-5 重复代码三组**（合计 S–M）[OK] 部分完成（83d48e6）
  ① [OK] `formatDate` 移入 `lib/chartLabel.ts`，HistoryPage/ProfilePage 复用；ProfilePage 的重复 `genderText` 一并收敛。
  ② [NO] 不抽 `usePageData`：5 个候选页面的鉴权守卫/错误展示/分页形状各异，强行抽象不满足单一职责标准（CLAUDE.md §5.1）。顺带修了 CompatibilityHistoryPage 加载缺 `.catch` 的静默吞错（P0-1 同款，当时排查遗漏）。
  ③ ⏳ ShareCard↔CompatibilityShareCard、PrintLayout↔CompatibilityPrintLayout 同构组件对 → 留给 P1-5 拆分时一并处理。

- [x] **P2-6 神煞注解加载失败完全无声**（S）[OK] 已修（1298b93）
  可选数据失败可以容忍，但连 console.warn 都没有，线上排查困难。
  证据：`frontend/src/pages/ResultPage.tsx:381-389`。

- [x] **P2-7 报告 tab 切换不滚动**（S）[OK] 已修（57076c5）
  原版/润色版切换后视口停在原处。做法：切换后 scrollIntoView 回报告区顶部，scroll-margin-top 避开 fixed 导航栏。

- [x] **P2-8 BottomNav 高亮判断粗糙**（S）[OK] 已修（57076c5）
  做法：抽成 `activeTab(pathname)` 显式映射，补上 `/result`→测算、`/settings`→我的 的归属。

- [x] **P2-9 lint 残留 1 个 warning**（S，5 分钟）[OK] 已修（2f95ab7）
...
- [x] **P2-13 品牌 logo 上传失败需重选文件**（S）[OK] 确认成立 [OK] 已修（9f85558）
