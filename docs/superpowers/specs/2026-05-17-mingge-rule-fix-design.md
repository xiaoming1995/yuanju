...
> ## [WARN] 状态：已废弃（SUPERSEDED）
>
> 此方案的方向是 **"修补七优先级表的偏差"**。实施完毕后用 12 个专业命师标注的真实盘对照，发现命师**根本不使用七优先级表**，而是用另一套基于「四柱透干 + 通根强度评分」的方法。本方案对齐率仅 5/12。
>
> **接替本方案**：[`2026-05-17-mingge-professional-rules-design.md`](./2026-05-17-mingge-professional-rules-design.md) — 整体重写 `DetectMingGe`，实现命师 6 条隐性规则，对齐率 12/12。
>
> 本文件保留以记录失败方向，避免日后重蹈覆辙。
